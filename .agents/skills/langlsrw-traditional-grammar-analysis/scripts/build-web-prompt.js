...
function buildWebPrompt(source) {
  ...
  return [
    "// Generated from langlsrw-traditional-grammar-analysis/references/grammar-prompt.txt.",
...
function run(args) {
  ...
  if (!outputPath) {
    ...
    const localSkill = path.join(projectRoot, ".agents", "skills", "langlsrw-traditional-grammar-analysis");
    if (path.resolve(localSkill) !== skillRoot || !fs.existsSync(path.join(projectRoot, "src", "app.js"))) {
      throw new Error("Standalone skill: specify the browser destination with --output.");
    }
    outputPath = path.join(projectRoot, "src", "generated", "grammar-prompt.js");
  }
