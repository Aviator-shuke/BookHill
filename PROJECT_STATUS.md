...
- The toolbar above the listening practice area has a `句库` quick-switch dropdown (常用句库 / 用户收藏) as its first control, so switching the active practice source no longer requires opening the library dialog. The sentence counter next to it is an editable jump-to-sentence input rather than plain text.
...
- The `句库` picker offers the built-in common library, a `用户收藏` library built from the learner's starred sentences (searchable, loads into practice via the same "使用此句库" flow), and custom import/paste. The other placeholder categories (情景/语法/等级/短语句库) still have no data.
